...
public partial class AIvAI : Control
{
	...
	public async void PlayAiVsAi()
	{
		...
		while (true)
		{
			...
			status.Text = "Requesting game state";
			...
			if (playStateResponse.StatusCode != System.Net.HttpStatusCode.OK || playStateResponse.StatusCode == System.Net.HttpStatusCode.InternalServerError)
			{
				...
				status.Text = "Sleeping and trying again...";
			...
			gameInfo.Text = $"Game id: {game_id}   Action id: {action_id}\nState: {state}";
			...
			***************************************************************************************/
			...
			NeuralNetStrategy.Move bestMove;
			try
			{
				bestMove = NeuralNetStrategy.ChooseBestMove(currentGame, activePlayer);
			...
			status.Text = $"Move chosen: {bestMove}";
			...
			***************************************************************************************/
			...
			{
				status.Text = $"Problem sending move. Sleeping and trying again...\n{submitResponse.StatusCode}";
	...
	public override void _Ready()
	{
		...
		gameInfo = GetNode<Label>("%Game Info");
	}
	...
	public void _on_start_btn_pressed()
	{
		try
		{
			status.Text = "Starting play loop";
			PlayAiVsAi();
		}
		catch (HttpRequestException ex)
		{
			status.Text = "Request to softserve failed";
			GD.PrintErr($"Request to softserve failed: {ex.Message}");
		}
	}
